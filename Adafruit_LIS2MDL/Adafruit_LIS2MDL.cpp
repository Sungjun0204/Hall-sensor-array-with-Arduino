...
// continuous mode로 설정 & ODR 100Hz 설정
void Adafruit_LIS2MDL::CFG_REG_A(void) {
    if (spi_dev) {
        Adafruit_BusIO_Register cfg_reg =
            Adafruit_BusIO_Register(spi_dev, 0x60, ADDRBIT8_HIGH_TOREAD);
        cfg_reg.write(0b00001100);
        delay(10);
    }
}

void Adafruit_LIS2MDL::CFG_REG_B(void) {
    if (spi_dev) {
        Adafruit_BusIO_Register cfg_reg =
            Adafruit_BusIO_Register(spi_dev, 0x61, ADDRBIT8_HIGH_TOREAD);
        cfg_reg.write(0b00000011);
        delay(10);
    }
}
